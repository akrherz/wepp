...
c
     ...
     1    ipart,i1,i2, luntc, itcch, itcimp, itc
c
...
c
      ...
      luntc = 0
      open(unit=65,file='tc.txt',status='old',err=401)
      luntc = 1
      close(65)
  401 continue        
      if (luntc.ne.0) then
        open(unit=65,file='tc_out.txt',status='unknown')
        write(65,3000)
        write(65,3001)
        write(65,3002)
      endif
...
c
     ...
     1              .or.(nomelt.ne.0).or.(qbase(ichan).gt.0.)) then
c
...
c Added by L. Wang, 9/11/2009.
                  if(runvol(ielmt) > 0.001) then
c End adding.
cd    Following line modified by S. Dun Temperarily 01/14/2004
c                  do 155 ipart = 1, npart
c                        tgsd(ipart,ielmt) = 0.0
c155               continue
c
...
c Added by L. Wang, 9/11/2009.
                   ...
                   if(ipeak>2) then
CAS fix for channel sediment bug (absence o initialization caused sediment carry over to following storms
                  do 156 ipart = 1, npart
                        tgsd(ipart,ielmt) = 0.0
156               continue
CAS end
                     call wshchr
...
c End adding.
                ...
                if (ielmt.eq.nelmt) then
                    if (runvol(ielmt).gt.0.005) then
                      if (luntc.eq.1) then
                        itcch = 0
                        itcimp = 0
                        do 173 itc = nhill + 1, nelmt
                           if (elmt(itc).eq.2) then
                               itcch = itcch + 1
                               write (65,2800) itc, itcch,
     1                            sdate, year, runvol(itc),htcc(itc),
     1                            (stmdur/3600.0),
     1                            (stmdur/3600.0)*timep
                           else
                              if (elmt(itc).eq.3) then
                                 itcimp = itcimp + 1
                                 write (65,2900) itc, itcimp,
     1                            sdate, year, runvol(itc),htcc(itc),
     1                            (stmdur/3600.0),
     1                            (stmdur/3600.0)*timep
                              endif

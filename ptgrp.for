...
CASnew Added by A. Srivastava for mgtopt(nowcrp,iplane).eq.4
      include 'csenes.inc'
      include 'cnew.inc'
CASnew
c
...
CAS Uncommented by A. Srivastava to do cutting with and without removal.
CAS mgtopt(nowcrp,iplane).eq.4 will use new residue routine (resup2.for)
         if (mgtopt(nowcrp,iplane).eq.1 .or.
     1        mgtopt(nowcrp,iplane).eq.4) then
c
c         Management Option 1 (cutting with removal)
CAS          if (mgtopt(nowcrp,iplane).eq.1) then   !!commented by A. Srivastava
...
c             cover.   dcf  4/25/94
c
CAS Added to do cut perennial without removal
...
c         *** L2 ELSEIF ***
          else if (mgtopt(nowcrp,iplane).eq.2.or.
     1             mgtopt(nowcrp,iplane).eq.5) then
c
c           *** L3 IF ***
...
CASnew Added a condition
                    if(mgtopt(nowcrp,iplane).eq.2) then
     ...
     1                area(iplane)
                  vdmt(iplane) = vdmt(iplane) - tlfood
                    elseif(mgtopt(nowcrp,iplane).eq.5) then
                  vdmt(iplane) = vdmt(iplane)*
     1                        (1-perremo(ngraz(iplane),nowcrp,iplane))
                    endif
                  if (vdmt(iplane).lt.0.0) vdmt(iplane) = 0.0
CASnew Update LAI from vdmt
                  if (vdmt(iplane).eq.0.0) then
                      lai(iplane) = 0.0
                      cancov(iplane) = 0.0
                      canhgt(iplane) = 0.0
                      sumgdd(iplane) = 0.0
...
c
c           *** L3 ELSEIF ***
          if (ngraz(iplane).gt.0) then
          if (sdate.eq.gday(ngraz(iplane),nowcrp,iplane)) then
              if (vdmt(iplane).gt.critvm(itype(nowcrp,iplane))) then
                  vdmttmp = 0.0
                  vdmttmp = vdmt(iplane)
                  vdmt(iplane) = vdmt(iplane)*
     1                        (1-perremo3(ngraz(iplane),nowcrp,iplane))
                  if (vdmt(iplane).lt.0.0) vdmt(iplane) = 0.0
CASnew Update LAI
                  if (vdmt(iplane).eq.0.0) then
                      lai(iplane) = 0.0
...
CASnew
      if(mgtopt(nowcrp,iplane).eq.7) then
            if (ngraz(iplane).eq.0) then
              ngraz(iplane) = 1
            endif
c
c           *** L3 ELSEIF ***
          ...
          if (sdate.eq.gday(ngraz(iplane),nowcrp,iplane)) then
              if (vdmt(iplane).gt.critvm(itype(nowcrp,iplane))) then
                  ...
                  vdmttmp = vdmt(iplane)
                  vdmt(iplane) = vdmt(iplane)*
     1                        (1-perremo5(ngraz(iplane),nowcrp,iplane))
...
CAS Added to do cut perennial without removal
              pyld = vdmttmp*perremo5(ngraz(iplane),nowcrp,iplane)
              cutflg = 14
              call resup2(nowcrp,cutflg,pyld)
CAS end
              endif
...
c
        ...
        lai(iplane) = 0.0
        ncount(iplane) = 0
CASnew On planting date resetting cutflg equal to zero for perennials.
        cutflg = 0
CASnew
CASnew Added by A. Srivastava to implement release cover crop for perennials 7/13/2018
...
c
 ...
 !!    1    '(kg/m**2)')
 1000 format (' CUTTING date ',i3,' plane ',i4,' yield ',f8.3,
     1    '(kg/m**2)')
...
CASnew
 ...
 !!    1    ' (kg/m**2)')
 1200 format (' CUTTING date ',i3,' channel ',i4,' yield ',f8.3,
     1    ' (kg/m**2)')
      end
...
c
     ...
     1    tmpdep(mxhill),drrunv,tmpdrf(mxhill),tmpdrv(mxhill)
c Added by L. Wang, 12/04/2009.
      real qptsum, qpsum
c End adding.     
c
...
c
      ...
      if (istat.eq.1) then
c         calculate a tile drainage volume
          if (drainq(nplane).gt.0.0) then
             drrunv = drainq(nplane) * slplen(nplane)
     1        * fwidth(nplane) 
          endif
c
...
c     Added tile drainage (drainq and drrunv)to pass file - 3/10/2010 - jrf
        write (48,1000) event(1), year, day, dur, tcs, oalpha,
     1        runoff(nplane), runvol(nplane),sbrunf(nplane), 
     1        sbrunv(nplane), drainq(nplane), drrunv, peakro(nplane) *
...
cx
          ...
          sbrunv(nplane)=sbrunf(nplane) * slplen(nplane)
     1        * fwidth(nplane)
          if ((sbrunv(nplane).gt.0.01).or.(drainq(nplane).gt.0.0)) then
c    Added tile drainage (drainq and drrunv)to pass file - 3/10/2010 - jrf                  
             event(1)= 'SUBEVENT' 
             write(48,1000) event(1), year, day, sbrunf(nplane), 
     1            sbrunv(nplane),drainq(nplane), drrunv
          else
             event(1) = 'NO EVENT'
...
cx    End Modifying.
c
        else if (drainq(nplane).gt.0.0) then
          event(1) = 'SUBEVENT'
          sbrunv(nplane) = 0.0
          sbrunf(nplane) = 0.0
c    Added tile drainage (drainq and drrunv)to pass file - 3/10/2010 - jrf           
...
c
c     This reads in the individual hillslope pass files and writes out
c     a watershed pass file.
      if (istat.eq.2) then
c
        do 10 i = 1, nhill
c
...
c
     ...
     1        harea(i)
c
c         moved assignment here to work with older FORTRAN semantics jrf 2/16/2012
          hsarea(i) = harea(i)
   20   continue
c
        write (49,1800)
...
cx    End adding
c     Added tile drainage 3/10/2010 - jrf
              write (49,1400) (tmpdrf(k), k = 1, nhill)
              write (49,1400) (tmpdrv(k), k = 1, nhill)
...
cx End adding
 ...
 1700 format ('HILLSLOPE ',i4,3x,a,5(e11.5,1x),5x,e10.5)

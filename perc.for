...
cd    Modified by DSH 09/19/2002
        if(k1.eq.nsl(iplane)) then
           ...
           if (ui_run.ne.1) then
             if (ssc(k1+1,iplane).gt.0.0) then
...
cd    Modified by S. Dun for Erin's request July 22, 2008
            if (ssc(k1+1,iplane).gt.0.0) then
               sscz =(dg(k1,iplane)+ ui_bdrkth(iplane))/
     1            (dg(k1,iplane)/ssc(k1,iplane)
     1            + ui_bdrkth(iplane)/ssc(k1+1,iplane))
            else
               sscz = 0.0
            endif
           endif

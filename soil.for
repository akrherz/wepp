...
CAS To test values 
      !!write(*,*)sdate,year,i1,i2,mdate(i1,i2)
CAS End modifying.
c
c           *** M3 IF ***
...
c             ------------ initial ridge height and interval after tillage operation

              if ((rh(iplane).lt.rho(i1,i2)).and.
     1                        (rho(i1,i2).lt.998.00)) then !! CAS - Flag 999.00 is used for centain operation that do not change ridge height and interval.
                  rhinit(iplane) = rho(i1,i2)
                  riinit(iplane) = rinter(i1,i2) ! Updates ridge interval after operations.
              else
                  rhinit(iplane) = rho(i1,i2)                  
              endif
     1            
              if (rhinit(iplane).lt.0.006) then
                  rhinit(iplane) = 0.006 
                  riinit(iplane) = 0.05 ! Lower limit for implement ridge interval.
              endif
...
CAS
     ...
     1                        (sdate.gt.tildate(iplane))) then
                  ...
                  if(cnfail(1) .eq. 1) then
                      contrs(nowcrp,iplane) = 0 ! Switching off contours if contours failed the previous day
                  else if (cnfail(iplane) .eq. 1) then
                      contrs(nowcrp,iplane) = 0
                  endif
              endif
999           continue
      endif
...
CAS Added by A. Srivastava to test values. 3/21/2016
     !! write(*,2000)sdate,year,rfcum(iplane),daycnt(iplane,1),
     !!1        daycnt(iplane,2),daydis(iplane),rhinit(iplane),
     !!1        rho(i1,i2),rh(iplane),riinit(iplane)
      !!write(*,2000)sdate,year,tildate(iplane)
CAS
c
c---------------------------------------------------------------------------
...
CAS Start
     ...
     1                rh(iplane) .lt. 0.0508) then
                      contrs(nowcrp,iplane) = 0 ! Switching off contours if contours failed the previous day
                      write(6,*)'CONTOUR ROUTING DISABLED ON PLANE', 
     1            iplane,' ON DAY ',sdate, 'RIDGE HEIGHT LESS 
     1            THAN 0.0508 M (2 INCH)'
                end if
              endif
      endif
CAS Ends
...
c
CAS Commented by A. Srivastava 3/21/2016
        if ((rh(iplane)/(rspace(iplane)/2.)).gt.avgslp(iplane)) then ! rspace should be ridge interval
          denom = rh(iplane) /
...
c
CAS Added by A. Srivastava to test values. 3/21/2016
     !!!! write(66,2000)sdate,year,rh(iplane),rspace(iplane),riinit(iplane),
     !!!!1    denom,ckiasa,kiadjf(iplane),avgslp(iplane)
CAS
c-------------------------------------------------------------

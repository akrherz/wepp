...
CAS To correct water balance error
        if (wntdur .eq. 24.0) wnttim = 1.0
CAS
c
c       condtion L2
...
c
c       condition L3
CAS With breakpoint climate vercli equal to 0.0, the model defaults partitioning using dew-point temp. 
CAS So, commented lines for rain-snow partition by dew point temp. 
CAS If we still want to use this option, we would want to use other ways of flag. A. Srivastava 2/19/2021																		  
CAS        if (vercli .ne. 0) then 

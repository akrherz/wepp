...
c       *************************************************************
c
CAS Putting oratea inside the loop to decompose flat residues in three pools
CAS with different decomposition rates.
          if(nowres .lt. 3) then
              neworatea(nowres,iplane) = oratea(iresd(nowres,iplane))
              newcf(nowres) = cf(iresd(nowres,iplane))
          endif
CASoriginal          tmpvr1 = oratea(iresd(nowres,iplane)) * ferind * pszind
          
          tmpvr1 = neworatea(nowres,iplane) * ferind * pszind
...
CAS testing
     !!     write(63,3000)sdate,year,itype(nowcrp,iplane),nowres,
     !!1    iresd(nowres,iplane),
     !!1    rmogt(nowres,iplane), smrm(nowres,iplane),
     !!1    oratea(iresd(nowres,iplane)),neworatea(nowres,iplane),
     !!1    cf(iresd(nowres,iplane)),newcf(nowres)
CAS
        
   20 continue
      

CAS Reverted back to what it was earlier. i.e. flat and submerged decomposition rates
CAS are same.
c       5/2/2018 - ALL below ground residue (buried and dead roots) now
c       uses the ORATER decomposition var. This allows the surface and
c       buried residues to decompose at different rates.
c
...
c       *************************************************************
c
CAS Note: Root decomposition 
      do 25 nowres = 1, numresrtm
CAS Commented by A. Srivastava
        !!tmpvr2 = orater(iresd(1,iplane)) * ferind * pszind
CAS Changed first subscript of iresd from 1 to nowres.
CAS        tmpvr2 = orater(iroot(nowres,iplane)) * ferind * pszind
          if(nowres .lt. 3) then
              neworater(nowres,iplane) = orater(iroot(nowres,iplane))
          endif
        tmpvr2 = neworater(nowres,iplane) * ferind * pszind

        env = exp(-benvin(nowres,iplane)*tmpvr2)
        rtm(nowres,iplane) = rtm(nowres,iplane) * env
CAS testing
     !!     write(63,3000)sdate,year,itype(nowcrp,iplane),nowres,
     !!1    iroot(nowres,iplane),orater(iroot(nowres,iplane)),
     !!1    rtm(nowres,iplane),neworater(nowres,iplane)
CAS
...
CAS Commented this whole section and moved into M1 if section.
CAS This was done to allow shred/cut,burn, herbicide, silage in
CAS one yearly scenario using mdate in operations section.
      if (manver .le. 2016.3) then
c     Burning
c
...
CAS        do 40 nowres = 1, numres
c         -------- add mass of cut residue to ridge & rill residue masses
CAS          rilrm(nowres,iplane) = rilrm(nowres,iplane) + tmpvr4
CAS          rigrm(nowres,iplane) = rigrm(nowres,iplane) + tmpvr4
c         -------- add mass of cut residue to residue mass on the ground
c         (WEPP Equation 8.4.14)
CAS          rmogt(nowres,iplane) = rmogt(nowres,iplane) + tmpvr4
CAS   40   continue
CAS
c         -------- add mass of cut residue to ridge & rill residue masses
          rilrm(1,iplane) = rilrm(1,iplane) + tmpvr4
          rigrm(1,iplane) = rigrm(1,iplane) + tmpvr4
c         -------- add mass of cut residue to residue mass on the ground
c         (WEPP Equation 8.4.14)
          rmogt(1,iplane) = rmogt(1,iplane) + tmpvr4
CAS
c
c     *** L1 ELSE-IF ***
...
CAS Computing net residue buried that is buried minus resurfaced
              flrcov = (1.0-exp(-newcf(nowres)*
     1            rigrm(nowres,iplane))) * (1.0-
     1    (mfo(i1,i2,iresd(nowres,iplane))-
     1    resur(i1,i2,iresd(nowres,iplane))))
CAS
...
!!CAS
!!              write(63,5000)sdate,year,nowres,
...
!!              do 85 nowres = 1, numres
!!CAS Submerged residue amount
!!              buried = smrm(nowres,iplane)
!!     !!         write(63,5000) sdate,year,nowres,buried, 
!!     !!1            resur(i1,i2,iresd(nowres,iplane)),smrm(nowres,iplane),
!!     !!1            rigrm(nowres,iplane),rilrm(nowres,iplane),
!!     !!1            rmogt(nowres,iplane)
!!CAS Fraction of submerged amount removed
!!              smrm(nowres,iplane) = smrm(nowres,iplane)*
!!     1                        (1-resur(i1,i2,iresd(nowres,iplane)))
!!CAS Updating flat residue pool. Adding fraction submerged removed to flat pool.
!!              rigrm(nowres,iplane) = rigrm(nowres,iplane) + 
!!     1                        (buried-smrm(nowres,iplane))
!!              rilrm(nowres,iplane) = rilrm(nowres,iplane) + 
!!     1                        (buried-smrm(nowres,iplane))
!!              rmogt(nowres,iplane) = rmogt(nowres,iplane) + 
!!     1            buried*resur(i1,i2,iresd(nowres,iplane))
!!CAS End updating flat residue pool
!!     !!         write(63,5000) sdate,year,nowres,buried, 
!!     !!1            resur(i1,i2,iresd(nowres,iplane)),smrm(nowres,iplane),
!!     !!1            rigrm(nowres,iplane),rilrm(nowres,iplane),
!!     !!1            rmogt(nowres,iplane)
...
CAS for OLD RESIDUE POOL following residue additions. 
          rmogt2 = 0.0
          rmogt3 = 0.0
...
CAS
     ...
     1        (rmogt2+rmogt3)
              ...
              newcf(3) = cfwt
CAS testing
          !!write(63,4000)sdate, year, rmogt2,neworatea(2,iplane),newcf(2)
          !!write(63,4000)sdate, year, rmogt3,neworatea(3,iplane),newcf(3)
          !!write(63,4000)sdate, year, orateawt, cfwt
          !!write(63,*) cntres
CAS end testing
      endif
...
c
      if (manver .ge. 2016.3) then
          if (resman(i1,i2).eq.15) then
!!CAS for tillflg = 0, switch of burning; otherwise on.
!!          tillflg = 1
!!          if(jdplt(nowcrp,iplane).lt.jdharv(nowcrp,iplane)) then
!!              if(mdate(i1,i2).gt.jdplt(nowcrp,iplane).and.
!!     1            mdate(i1,i2).lt.jdharv(nowcrp,iplane)) then
!!                  tillflg = 0
!!              endif
!!          else
!!              if((mdate(i1,i2).gt.jdplt(nowcrp,iplane).and.
!!     1            mdate(i1,i2).le.366).or.
!!     1          (mdate(i1,i2).ge.1 .and. mdate(i1,i2).lt.
!!     1                    jdplt(nowcrp,iplane))) then
!!                  tillflg = 0
!!              endif
!!          endif
!!          if(imngmt(nowcrp,iplane).eq.3) tillflg = 1
!!          !!if(imngmt(nowcrp,iplane).eq.2) tillflg = 0 !! burning doesn't effect perennial 
!!                                                       !! crop growth
!!          if(tillflg .eq. 0) then
!!              write(6,*)'Burning operation switched off on day',sdate,
!!     1                   'year',year
!!     !!         write(*,*)'Burning operation switched off on day',sdate,
!!     !!1                   'year',year
!!          endif
!!          if(tillflg.eq.1) then
              jdburn(nowcrp,iplane) = 0
              jdcut(nowcrp,iplane) = 0

              jdburn(nowcrp,iplane) = mdate(i1,i2)
     !!         write(36,*)mdate(i1,i2),jdplt(nowcrp,iplane),
     !!1    jdharv(nowcrp,iplane),jdburn(nowcrp,iplane),
     !!1    jdcut(nowcrp,iplane)
c       ------ fraction of residue mass remaining
            tmpvr3 = 1.0 - fbrnog(i1,i2)
            do 120 nowres = 1, numres
c         -------- update ridge & rill residue masses
              rigrm(nowres,iplane) = rigrm(nowres,iplane) * tmpvr3
              rilrm(nowres,iplane) = rilrm(nowres,iplane) * tmpvr3
c         -------- update residue mass on the ground
...
c       ------ update residue mass above the ground
c       (WEPP Equation 8.4.11)
            rmagt(iplane) = rmagt(iplane) * (1.-fbrnag(i1,i2))
...
c     Standing residue cut
c
          if (resman(i1,i2).eq.14) then
!!CAS for tillflg = 0, switch of shredding; otherwise on.
!!          tillflg = 1
!!          if(jdplt(nowcrp,iplane).lt.jdharv(nowcrp,iplane)) then
!!              if(mdate(i1,i2).gt.jdplt(nowcrp,iplane).and.
!!     1            mdate(i1,i2).lt.jdharv(nowcrp,iplane)) then
...
!!          endif
!!          if(imngmt(nowcrp,iplane).eq.3) tillflg = 1
!!          !!if(imngmt(nowcrp,iplane).eq.2) tillflg = 0 !! shredding doesn't effect perennial 
!!                                                       !! crop growth
!!          if(tillflg .eq. 0) then
!!              write(6,*)'Shredding operation switched off on day',sdate
!!     1                   ,'year',year
!!     !!         write(*,*)'Shredding operation switched off on day',sdate,
!!     !!1                   'year',year
!!          endif
          !!if(tillflg.eq.1) then
          jdburn(nowcrp,iplane) = 0
          jdcut(nowcrp,iplane) = 0
          jdcut(nowcrp,iplane) = mdate(i1,i2)
     !!     write(36,*)mdate(i1,i2),jdplt(nowcrp,iplane),
     !!1    jdharv(nowcrp,iplane),jdburn(nowcrp,iplane),
     !!1    jdcut(nowcrp,iplane)
c       ------ "remember" RMAGT
...
c       ------ re-compute standing residue mass
c       (WEPP Equation 8.4.13)
        rmagt(iplane) = rmagt(iplane) * (1.-frcut(i1,i2))
c       ------ compute decrease in standing residue mass
        tmpvr4 = rmagy - rmagt(iplane)
CAS Commented the loop. The shredded residue was added to 
CAS all three pools causing residue balance error
CAS        do 130 nowres = 1, numres
c         -------- add mass of cut residue to ridge & rill residue masses
CAS          rilrm(nowres,iplane) = rilrm(nowres,iplane) + tmpvr4
CAS          rigrm(nowres,iplane) = rigrm(nowres,iplane) + tmpvr4
c         -------- add mass of cut residue to residue mass on the ground
...
c
CAS   Silage...
      if(resman(i1,i2).eq.16) then
          jdslge(nowcrp,iplane) = mdate(i1,i2)
          goto 999
      endif
CAS   Herbicide...
      if(resman(i1,i2).eq.17) then
          jdherb(nowcrp,iplane) = mdate(i1,i2)
...
c
 ...
 1100 format(1x,f4.1,'% RESIDUE REMOVED FROM PLANE '
     1    ,i2,', WITH DISTURBANCE, ON DAY',i4)
CAS
!!2000  format(1x,4I4,7(1x,F10.4))
CAS
CAS
 !!3000    format(1x,5(1x,i5),15(1x,f16.5))
 !!4000    format(1x,2(1x,i5),15(1x,f16.5))
 !!5000    format(1x,3(1x,i5),15(1x,f16.5))
CAS
      end

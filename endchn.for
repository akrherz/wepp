...
c
     ...
     1    tgs(mxelem), tppm, trunsi, xchn, conv, sdr, wdet,tothill
c
...
c
     ...
     1    (dept(k)/nyear) * (-1), hsedt(k)/nyear, wsarea(k)/10000.0
        wdet = wdet + dett(k)
   10 continue
c
      tothill = wdet
...
c
     ...
     1      (cdett(ielmt)*conv)/nyear,
     1      tronvt(ielmt)/nyear, sbrvtt(ielmt)/nyear,
     1      wsarea(ielmt)/10000.0
...
c
     ...
     1        trunt(nelmt)/nyear, 
     1        (tothill/1000)/nyear,((wdet-tothill)/1000)/nyear,
     1        (tgs(nelmt) * conv/1000.)/nyear,
...
c
 ...
 1100 format (/17x,'Runoff',8x,'Subrunoff',5x,'Soil',10x,'Sediment',6x,
     1    'Sediment',6x,'Hillslope',/,17x,
     1    'Volume',8x,'Volume',8x,'Loss',10x,'Deposition',4x,'Yield',
     1    9x,'Area',/,'Hillslopes',7x,'(m^3)',9x,'(m^3)',9x,'(kg)',10x,
     1    '(kg)',10x,'(kg)',8x,'(ha)',/,10('-'),7x,6(10('-'),4x))
...
cd     1    '(tonne/yr)',/,12('-'),5x,2(10('-'),4x),/)
 1200 format (///'Channels',9x,'Discharge',5x,'Sediment',8x,'Soil',10x,
     1        'Upland',8x,'Subsuface Flow',3x,'Contributing'/,
     1        'and',14x,'Volume',8x,'Yield',11x,'Loss',10x,'Charge',8x,
     1        'Volume',10x,'Area'/,'Impoundments',5x,'(m^3)',9x,
     1         '(tonne)',9x,'(kg)',10x,'(m^3)',9x,'(m^3)',12x,'(ha)'/,
     1         12('-'),5x,2(10('-'),4x),2x,3(10('-'),3x),7x,10('-')/)
 1300 format ('Hill    ',i4,5(2x,f12.2),2x,f8.2)
 1400 format ('Channel ',i4,1x,f10.1,2x,f8.1,2x,f23.1,2x,
     1         f10.1,2x,f8.1,f8.2)
 1500 format ('Impoundment',1x,i2,1x,2(f9.1,5x))
 1600 format (/,i4,' storms produced ',f12.2,' mm. of rainfall on',
     1    ' an AVERAGE ANNUAL basis')
 1700 format (/,i4,' events produced ',f12.2,' mm. of runoff',/,5x,
 ...
 2500 format (/,' Average Annual Delivery From Channel Outlet:',/,
     ...
     1   '      = ',f28.0,'   m^3/yr',/,
     1   'Avg. Ann. total hillslope soil loss                 =',
     1   f30.1,1x,' tonnes/yr',/,
     1   'Avg. Ann. total channel soil loss                   =',
     1   f30.1,1x,' tonnes/yr',/,

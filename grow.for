...
c     + + + LOCAL VARIABLES + + +
     ...
     1    frst, veg, wght1,rtdmin,exparg,exptrm, bbeadj, beadj, ruedec1
      logical doStuff 
c
c     + + + LOCAL DEFINITIONS + + +
...
c
CAS Previous Commented by A. Srivastava
...
CAS Not allowing perennials to grow if current canopy height reaches max. canopy ht.
CAS This condition is not for regrowing trees e.g. conifers, hardwoods, woody trees etc.
CAS Hence, added jdsene condition below. 10/1/2019 A. Srivastava
      doStuff = .FALSE.
     !! if(imngmt(nowcrp,iplane).eq.2 .and. 
     !!1    jdsene(nowcrp,iplane).eq.0 .and.
     !!1    canhgt(iplane).gt.hmax(itype(nowcrp,iplane))-0.000001) then
     !!     canhgt(iplane) = hmax(itype(nowcrp,iplane)) + 0.000001
     !! endif
     
      if (lanuse(iplane).eq.1) then
         if ((imngmt(nowcrp,iplane).eq.1).and.
     1        (sumgdd(iplane).ge.crit(itype(nowcrp,iplane)))) then
              doStuff = .TRUE. !annual
         else if ((imngmt(nowcrp,iplane).eq.2).and.
     1        (sumgdd(iplane).ge.crit(itype(nowcrp,iplane)))) then
              if (jdsene(nowcrp,iplane).eq.0) then
                  if(canhgt(iplane).lt.hmax(itype(nowcrp,iplane))) then
                      doStuff = .TRUE. !perennial annual
                  endif
              else
                      doStuff = .TRUE. !perennial forest
              endif
         endif
      else if ((lanuse(iplane).eq.2).and.
     1        (tmnavg.ge.btemp(itype(nowcrp,iplane)))) then
              doStuff = .TRUE.  !rangeland
      endif
      if (doStuff .eqv. .TRUE.) then
c
...
c DFM 
          if (co2run) then
c#######################################################################
c###        Begin plant growth adjustment based on CO2               ###
c###        Reza Savabi added the code on Dec. 23, 1994              ###
c###        Recoded by DFM 6th April 1999                            ### 
c###                                                                 ### 
c###        This section simulates the impact of changed atmospheric ###
c###        CO2 content on ddm, based on existing routines in EPIC.  ###
c###        See Stockle (1992) for more detail                       ###
c###                                                                 ###
c#######################################################################

            beadj = 100.0*co2/(co2+exp(wac21(itype(nowcrp,iplane)) 
     1        -co2*wac22(itype(nowcrp,iplane))))

            ruedec1 = amax1(vpd(itype(nowcrp,iplane))-1.0,-0.5)
            bbeadj = beadj-wavp(itype(nowcrp,iplane))*ruedec1

            ddm = 0.0001*bbeadj*par
...
c
c   jrf - if the harvest index represents a portion of the rootmass harvested (hitype=0)
c      then don't use this factor when calculating canopy cover because the harvest index for
c      the above ground portion would be 0.
c      
...
CAS
          if (rtd(iplane).gt.solthk(nsl(iplane),iplane)) then
                if(jdsene(nowcrp,iplane).eq.0) then
                      rtd(iplane) = solthk(nsl(iplane),iplane)
                else
                      rtd(iplane) = (solthk(nsl(iplane),iplane)
...
c           (1-hia). 
c   jrf - if the harvest index represents a portion of the rootmass harvested (hitype=0)
c      then don't use this factor when calculating veg material because the harvest index for
c      the above ground portion would be 0.
c
            if (hitype(itype(nowcrp,iplane)).eq.1) then
...
CAS Added by A. Srivastava 7/10/2018. This is to prevent cancov and vdmt to go negative.
              if (cancov(iplane).lt.0.0) cancov(iplane) = 0.0
